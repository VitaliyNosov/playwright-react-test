...
const CreditCheckFailed = ({ handleGotoPrev, handleGotoNext }) => {
  ...
  if (companyData.FailMessage && companyData.FailMessage != 'Ind Search') {
    return (
      <div className="custom-steps check-failed">
        <h3 className="subtitle subtitle-custom">We cannot identify your business. Please check you have:</h3>
        ...
        <div className="subtitle subtitle-custom">Would you like us to refer you to one of our lending partners?</div>
        <div className="form-field formCustomCheckBox">
          <div className="checkboxes checkboxes-custom" style={{ width: '100%', display: 'block', textAlign: 'center' }}>
            <input
              id={'agree'}
              checked={!checked}
              name={'doNotAgree'}
        ...
        <div>
          <div className={'d-flex bottom-actions align-center'}>
            <div className="d-flex">
              <button onClick={handleSubmit} className="primary-btn primary-btn-custom">
                Submit
              </button>
            </div>
          </div>
  ...
  return (
    <div className="custom-steps check-failed">
      <div>
        <div className="check-failed-color">
          We cannot provide you an UnLock Mastercard<sup>&reg;</sup> at this time.
        </div>
        <div className="my-4 check-failed-color">Good news, one of our lending partners could help you.</div>
        <div className="form-field formCustomCheckBox">
          <input
            id={'agree'}
            checked={checked}
            ...
            onChange={(e) => setChecked(e.target.checked)}
          />
          <span className="font-bold check-failed-color">Please check if you would like a lending partner to contact you.</span>
        </div>
        <div>
          <div className={'d-flex bottom-actions align-center'}>
            <div className="d-flex">
              <button onClick={handleSubmit} className="primary-btn custom-button">
                Submit
              </button>
            </div>
          </div>
